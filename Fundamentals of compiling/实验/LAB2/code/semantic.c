...
pType newType(Kind kind,...){
    ...
    assert(kind == BASIC || kind == ARRAY || kind == STRUCTURE ||
           kind == FUNCTION);
    switch (kind) {
        case BASIC: //基本类型int or float
            va_start(vaList, 1);
            p->u.basic = va_arg(vaList, BasicType);
            break;
        case ARRAY: //数组类型
            va_start(vaList, 2);
            p->u.array.elem = va_arg(vaList, pType); //数组类型
            p->u.array.size = va_arg(vaList, int);   //数组大小
            break;
        case STRUCTURE: //结构体类型
            va_start(vaList, 2);
            p->u.structure.structName = va_arg(vaList, char*); //结构体名字
            p->u.structure.field = va_arg(vaList, pFieldList); //结构体中的域链表
            break;
        case FUNCTION:
            va_start(vaList, 3);
            p->u.function.argc = va_arg(vaList, int); //参数列表个数
            p->u.function.argv = va_arg(vaList, pFieldList); //形参域链表
            p->u.function.returnType = va_arg(vaList, pType); //返回类型
            break;
    }
    va_end(vaList);
    return p;
...
void deleteType(pType type) {
    ...
    if(type->kind != BASIC && type->kind != ARRAY && type->kind != STRUCTURE && type->kind != FUNCTION){
            return;
     }
...
void traverseTree(pNode node) {
    ...
    if (!strcmp(node->name, "ExtDef"))
        //ExtDef：包含全局变量的定义、结构体的定义、数组、基本类型
        ExtDef(node); 
...
void ExtDef(pNode node) {
    assert(node != NULL);
    // ExtDef -> Specifier ExtDecList SEMI 数组或基本类型
    //         | Specifier SEMI  结构体
    //         | Specifier FunDec CompSt  函数
    pType specifierType = Specifier(node->child);
    char* secondName = node->child->next->name; //产生式右边的第二个文法符号
...
pType StructSpecifier(pNode node) {
    ...
    pNode t = node->child->next;  //t指向OptTag或者Tag
...
void FunDec(pNode node, pType returnType) {
    ...
    //         | ID LP RP
    pItem p = newItem(table->stack->curStackDepth,newFieldList(node->child->val,newType(FUNCTION, 0, NULL, copyType(returnType))));
...
void VarList(pNode node, pItem func) {
    ...
    //          | ParamDec
    addStackDepth(table->stack); //新增一个栈，进入一个新的域
    int argc = 0;
    ...
    minusStackDepth(table->stack); //回到原来的栈，也就是外层代码块中
}
...
void CompSt(pNode node, pType returnType) { //进入函数体中，也就是内层代码中
    assert(node != NULL);
    // CompSt -> LC DefList StmtList RC  
    // LC：{  RC: }

    addStackDepth(table->stack); // 栈++
    pNode temp = node->child->next;  // 指向第二个文法符号
    ...
    clearCurDepthStackList(table); //退出函数体，清空当前栈中的符号hash表，栈--
}
