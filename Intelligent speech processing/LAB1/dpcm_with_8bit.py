...
def compress(encoded_signal):
    ...
    compressed_data = np.zeros(int(num/2), dtype=np.int16)                       # 创建一个与编码信号一半长度相同的数组，用于存储压缩后的数据
    for i in range(0, num-1, 2):
        # 将奇数位置的值放在高四位，偶数位置的值放在低四位
        compressed_data[int(i/2)] = (np.int16(encoded_signal[i]) << 8) + np.int16(encoded_signal[i + 1])
...
def decompress(compressed_data):
    ...
    x = np.zeros(num_compressed * 2, dtype=np.int16)                             # x 用来存还原后的量化误差
    for i in range(num_compressed):
        # 解压缩，将高四位和低四位分别放入还原后的数组中
        x[2 * i + 1] = compressed_data[i] & np.int16(int('0000000011111111', 2))
        x[2 * i] = (compressed_data[i] >> 8) & np.int16(int('0000000011111111', 2))
    return x
...
def save_encoded_data_to_file(begin, encoded_signal, filename):
    with open('./dpc/' + filename + '_8bit.dpc', 'wb') as file:
        file.write(begin)
        file.write(encoded_signal)
            
def save_decoded_data_to_pcm(decoded_signal, filename, framerate=44100):
    with wave.open('./pcm/' + filename + '_8bit.dpc', 'w') as pcmfile:
...
def DPCM(file_name, factor):
    ...
    begin, encoded_data = DPCM_encode_quantized(wav_data, 8, factor)  
    compressed_data = compress(encoded_data)  
    save_encoded_data_to_file(begin, compressed_data, file_base_name)

    dpc_file = open('./dpc/' + file_base_name + '_8bit.dpc', "rb")

    begin = np.frombuffer(dpc_file.read(2), dtype = np.short)
    str_data = dpc_file.read()  
    dpc_data = np.frombuffer(str_data, dtype=np.int16)
    decompressed_data = decompress(dpc_data)
    decoded_data = DPCM_decode_quantized(begin, decompressed_data, 8, factor) 
