...
{
	/* 
	 * 判断是否是内置函数，如果是，执行并返回1；如果不是，返回0
 	 * 内置函数：quit, jobs, bg or fg 
	 */
	if(!strcmp(argv[0],"quit")){
        ...
        listjobs(jobs); 		/* 打印当前所有暂停的进程信息 */
        return 1;
    }

    else if(!strcmp(argv[0],"bg")||!strcmp(argv[0],"fg")){
        do_bgfg(argv);			/* 将作业放到后台或前台运行 */
        return 1;
    }
	else 
...
{
    /* 
	 * 调用fgpid函数，判断目标进程pid是否改变
     * 通过sigsuspend函数将当前进程挂起
	 */
	sigset_t mask,prev;
    sigemptyset(&mask);
...
void sigchld_handler(int sig) 
{
    int olderrno = errno;               /* 保存程序在运行中的错误代码 */
    int status;
    pid_t pid;
    sigset_t mask, prev;
    sigfillset(&mask);
    while((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0){
        /* 
         *     waitpid回收僵死进程，参数WHOANG|WUNTRACED代表立即返回，
         *     等待集合中的一个进程终止或收到SIGCONT而从停止状态重新开始，若无子进程终止返回0值
         *     用WIFEXITED、WIFSIGNALED、WIFSTOPPED宏函数判断进程结束状态
         *     显示异常情况及造成原因
         */
        if (WIFEXITED(status))               /* 进程正常结束 */
        {
            /* 
            *     为避免删除任务时shell捕获并处理其他信号
            *     阻塞所有信号的接收，直至任务被删除
            */
            sigprocmask(SIG_BLOCK, &mask, &prev);
            deletejob(jobs, pid);
        ...
        else if (WIFSIGNALED(status))        /* 进程异常结束 */
        {
        ...
        else if (WIFSTOPPED(status))         /* 进程处于暂停状态 */
        {
...
void sigint_handler(int sig) 
{
    ...
    if (pid == 0) 
        return;
    kill(-pid, SIGINT);       /* 向所有前台运行的进程所处进程组发送SIGINT信号 */
...
void sigtstp_handler(int sig) 
{
    ...
    p->state = ST;              /* 设置相应工作状态为ST */
    kill(-pid, SIGTSTP);        /* 向所有前台运行的进程所处进程组发送SIGTSTP信号 */
    printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(pid), pid, sig);
                                /*显示终止信息*/
    errno = olderrno;
    return;
