...
class AlbertModel(AlbertPreTrainedModel):
    ...
    Example::
        ...
        cls = sequence_output[:, 0, :].unsqueeze(1).unsqueeze(1)
        cls = cls.repeat(1, e1_e2_start.shape[0], e1_e2_start.shape[1], 1)

        buffer = []
        for i in range(cls.shape[0]): # iterate batch & collect
            cls_token = cls[i, i, :, :]
            v1v2 = torch.cat((cls_token[0], cls_token[1]))
            buffer.append(v1v2)
        del cls
        v1v2 = torch.stack([a for a in buffer], dim=0)
        del buffer
