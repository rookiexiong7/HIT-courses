...
pInterCode newInterCode(int kind, ...) {
    ...
    switch (kind) {
        ...
        case IR_WRITE_ADDR:
        case IR_CALL:
            va_start(vaList, 2); //left := call right
...
void printInterCode(FILE* fp, pInterCodeList interCodeList) {
    for (pInterCodes cur = interCodeList->head; cur != NULL; cur = cur->next) {
        ...
        if (fp == NULL) {
            ...
            switch (cur->code->kind) {
                case IR_LABEL:
                    fprintf(fp, "LABEL ");//LABEL x : 定义标号x。
                    printOp(fp, cur->code->u.oneOp.op);
                    fprintf(fp, " :");
                    break;
                case IR_FUNCTION://FUNCTION f : 定义函数f。
                    fprintf(fp, "FUNCTION ");
                    printOp(fp, cur->code->u.oneOp.op);
                    fprintf(fp, " :");
                    break;
                case IR_ASSIGN://x := y 赋值操作。
                    printOp(fp, cur->code->u.assign.left);
                    fprintf(fp, " := ");
                    printOp(fp, cur->code->u.assign.right);
                    break;
                case IR_ADD://x := y + z 加法操作
                    printOp(fp, cur->code->u.binOp.result);
                    fprintf(fp, " := ");
                    printOp(fp, cur->code->u.binOp.op1);
                    fprintf(fp, " + ");
                ...
                case IR_DIV://x := y / z 除法操作。
                    ...
                    printOp(fp, cur->code->u.binOp.op2);
                    break;
                case IR_GET_ADDR://x := &y 取y的地址赋给x。
                    printOp(fp, cur->code->u.assign.left);
                    fprintf(fp, " := &");
                    printOp(fp, cur->code->u.assign.right);
                    break;
                case IR_READ_ADDR://x := *y 取以y值为地址的内存单元的内容赋给x。
                    printOp(fp, cur->code->u.assign.left);
                    fprintf(fp, " := *");
                    printOp(fp, cur->code->u.assign.right);
                    break;
                case IR_WRITE_ADDR://*x := y 取y值赋给以x值为地址的内存单元。
                    fprintf(fp, "*");
                    printOp(fp, cur->code->u.assign.left);
                    fprintf(fp, " := ");
                    printOp(fp, cur->code->u.assign.right);
                    break;
                case IR_GOTO://GOTO x 无条件跳转至标号x。
                    fprintf(fp, "GOTO ");
                    printOp(fp, cur->code->u.oneOp.op);
                    break;
                case IR_IF_GOTO://IF x [relop] y GOTO z 如果x与y满足[relop]关系则跳转至标号z
                    fprintf(fp, "IF ");
                    printOp(fp, cur->code->u.ifGoto.x);
                    fprintf(fp, " ");
                    printOp(fp, cur->code->u.ifGoto.relop);
                    ...
                    printOp(fp, cur->code->u.ifGoto.z);
                    break;
                case IR_RETURN://RETURN x 退出当前函数并返回x值。
                    fprintf(fp, "RETURN ");
                    printOp(fp, cur->code->u.oneOp.op);
                    break;
                case IR_DEC://DEC x [size] 内存空间申请，大小为4的倍数。
                    fprintf(fp, "DEC ");
                    printOp(fp, cur->code->u.dec.op);
                    fprintf(fp, " ");
                    fprintf(fp, "%d", cur->code->u.dec.size);
                    break;
                case IR_ARG://ARG x 传实参x。
                    fprintf(fp, "ARG ");
                    printOp(fp, cur->code->u.oneOp.op);
                    break;
                case IR_CALL://x := CALL f 调用函数，并将其返回值赋给x
                    printOp(fp, cur->code->u.assign.left);
                    fprintf(fp, " := CALL ");
                    printOp(fp, cur->code->u.assign.right);
                    break;
                case IR_PARAM://PARAM x 函数参数声明。
                    fprintf(fp, "PARAM ");
                    printOp(fp, cur->code->u.oneOp.op);
                    break;
                case IR_READ://READ x 从控制台读取x的值。
                    fprintf(fp, "READ ");
                    printOp(fp, cur->code->u.oneOp.op);
                    break;
                case IR_WRITE://WRITE x 向控制台打印x的值
                    fprintf(fp, "WRITE ");
                    printOp(fp, cur->code->u.oneOp.op);
                    break;
            }
...
/* 
 * 根据 kind 的值，它使用 switch 语句执行不同的操作
...
/*
没有全局变量和类型定义，所以只需要处理FunDec CompSt
*/
void translateExtDef(pNode node) {
    ...
    // 因为没有全局变量使用，ExtDecList不涉及中间代码生成，类型声明也不涉及，所以只需要处理FunDec和CompSt
...
/*
函数体的中间代码生成：
1.变量创建语句:translateDefList(temp);
2.执行语句：translateStmtList(temp);
*/
...
void translateVarDec(pNode node, pOperand place) {
    ...
    if (!strcmp(node->child->name, "ID")) {
        ...
        if (type->kind == BASIC) {//假如这个变量是基础类型的变量，那么就不用tx来代替这个标识符
            if (place) {
                interCodeList->tempVarNum--;
...
void translateStmt(pNode node) {
    ...
    //        |IF LP Exp RP Stmt ELSE Stmt
        ...
        // Stmt -> IF LP Exp RP Stmt ELSE Stmt
            ...
            genInterCode(IR_GOTO, label3); //if中的语句执行完应该跳转的地址
            genInterCode(IR_LABEL, label2);//else后的语句
            translateStmt(stmt->next->next);
            genInterCode(IR_LABEL, label3);//ifelse结束后的语句开始位置
        }
...
void translateExp(pNode node, pOperand place) {
        ...
        // 数组和结构体访问
            ...
            if (!strcmp(node->child->next->name, "LB")) {
                ...
                if (node->child->child->next &&
                    ...
                    genInterCode(IR_MUL, offset, idx, width);//offset=idx*width;
                    // 如果是ID[Exp],则需要对ID取址
                    if (base->kind == OP_VARIABLE) {
                        // printf("非结构体数组访问\n");
                        target = newTemp();
                        genInterCode(IR_GET_ADDR, target, base);//target := &base
                    } else {
                        // printf("结构体数组访问\n");
                        target = base;
                    }
                    genInterCode(IR_ADD, place, target, offset); //place=target+offset
    ...
    //		| ID LP RP
        ...
        if (!strcmp(node->child->next->next->name, "Args")) {
            ...
            if (!strcmp(node->child->val, "write")) {//write(x)
                ...
                while (argTemp) {
                    ...
                    // 一般参数直接传值
                        ...
                        genInterCode(IR_ARG, argTemp->op); //arg x
                    }
                    argTemp = argTemp->next;
                }
                if (place) { //place不为空
                    genInterCode(IR_CALL, place, funcTemp); //place :=call funcTemp
                } else {//place为空
                    pOperand temp = newTemp(); //new一个temp变量，接收functemp的返回值
